...
public class C07_Polymorphism {	
	/*
	 	...
	 	- 업 캐스팅 되었던 자식 클래스의 인스턴스만이 다시 원래의 타입으로 돌아올 수 있다
	...
	public static void doWalk(Wolf wolf) {
		wolf.pet();
	}
	...
	public static void main(String[] args) {
		...
		Coffee c1 = (Coffee)new Americano();
		Coffee c2 = new CafeLatte();
		Coffee c3 = a1;
		
		// 업 캐스팅 되고 나면 Dog타입에만 존재하는 메서드인 walk()를 사용할 수 없다
		// 하지만 Wolf타입에도 함께 존재하는 메서드는 사용할 수 있다
		Dog dog1 = new Dog();
		Wolf dog2 = new Dog("잠시늑대가된개");
		
		dog1.walk();
		//dog2.walk(); // 업 캐스팅 되어서 사용 불가능
		
		dog1.pet(); // pet과 run은 부모 클래스에 있어서 가능하고 walk는 자식 클래스 안에만있어서 불가
		...
		// 업 캐스팅된 상태로도 해당 클래스의 인스턴스인지 검사가 가능하다
		Wolf wolf1 = new Dog();
		Wolf wolf2 = new Wolf();
