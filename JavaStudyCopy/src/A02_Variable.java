...
public class A02_Variable {
		...
		/*	
			...
			- 전기가 통하는 동안 데이터가 계속 유지되는 성질을 가지고 있는 저장 매체
		 	...
		 	- 주로 어떤 프로그램으로 '저장하기' 기능을 수행했을 때  
		 	  보조기억장치에 해당 데이터를 보관한다
		 	
		 	※메모리와 하드드라이브를 구분해둔 이유? 
		 	
		 	- 모든 종류의 입력은 CPU로 계산이 되는데
		 	  하드 드라이브 에서 CPU로 데이터가 가기엔 비효율적이라서
		 	  메모리 (RAM)을 통해서 CPU로 데이터를 보내는게 효율적이다	
		 	...
		 	- 원래 알고있던 =의 의미 : 왼쪽의 값과 오른쪽의 값이 같다 ex) 3+x=9 x=6
		 	- 프로그래밍 언어에서 =의 의미 : 왼쪽의 변수에 오른쪽의 값을 넣어라 (대입) int a = 1; a=1		 	
		 */	
	...
	public static void main(String[] args) {
		...
		//  ※ 변수를 선언한 후에 값을 한번도 대입하지 않은 경우 변수를 활용할 수 없다
		...
		// int com; //Duplicate local variable com 중복된 변수	
	}
