...
public class D02_HashSet {
	...
	*/
	public static void main(String[] args) {
		...
		animals.add("cat");
		animals.add("cat");
		animals.add("cat"); // 같은 값이 있으면 하나만 출력
		
		System.out.println(animals);
		
		// Set은 index가 없어서 get()하나씩 꺼내는 것은 불가능하다
		// 반복문을 활용해서 모두 꺼내야 한다
		
		// forEach : 내용을 하나씩 꺼내면서 반복하는 반복 문법
		...
		// 컬랙션끼리는 다른 컬랙션으로의 변환이 자유롭다
		// (생성자로 변환을 지원해준다)
		HashSet<String> set = new HashSet<String>(otherZoo);		
		...
		list.add("악어");
		list.add("악어");
		list.add("악어"); // hashset이 아니고 arraylist 라서 중복 출력됨
