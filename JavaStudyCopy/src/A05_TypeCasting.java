...
public class A05_TypeCasting {
	...
	public static void main(String[] args) {
		...
		//작은 타입 값이 큰 타입에 들어갈 떄는 별도의 조치 없이도 타입이 변한다
		byte b1 = 127;		
		short s1 = b1;
		int i1 = b1;
		long l1 = b1;
		
		// 큰 타입 값을 작은 타입에 넣을 때는 강제형변환으로 대입이 가능하지만
		// 값이 손상될 수도 있다
		int num = 200;		
		byte bnum = (byte)num;
	
		System.out.println(bnum); // -56 둥근 원 돈다 생각하면 좋음 
	
		double d1 = 123.123112312323123;
		float f1 = (float)d1;
		int num2 = (int)d1;
		System.out.println(f1);
		
		// 같은 2byte지만 서로 강제 타입캐스팅이 필요한 관계
		short snum = -30000;  // short = -32768 ~ 32767
		char ch1 = '박';		  // char  = 0~ 65535 
				
		// char에 short값을 넣을 떄는 음수값이 존재할 위험이 있으므로 강제형변환이 필요하다
		ch1 = (char)snum;
		
		// short에 char값을 넣을 떄는 32767을 초과하는 양수가 존재할 위험이 있다
		snum = (short)ch1;
