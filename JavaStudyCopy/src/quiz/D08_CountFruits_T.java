...
public class D08_CountFruits_T {
	...
	public static void main(String[] args) {
		...
		List<String> randomFruitList = 
				new ArrayList<>(Arrays.asList(randomFruits));
		
		System.out.println("# Collection.frequency를 사용");
		
		// 과일 9개 X frequency로 1000번씩 = 총 9000번 반복
		for (int i = 0; i < FRUITS.length; ++i) {
		...
		for (int i = 0; i < randomFruits.length; ++i) {
			...
			// 아직 없는 과일이라면 get()을 했을 때 null이 나온다
			// 이미 맵에 등록된적 있는 과일이라면 개수가 나온다
			// int가 아닌 Integer를 쓰는 이유 : int에는 null을 못쓴다
