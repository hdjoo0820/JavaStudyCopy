public class C02_OOP {
	/*
		...
		- 객체 내부의 변수는 현재 상태를 나타내고 
		 ...
		 - 클래스를 통해 만들어내는 실체를 인스턴스(intstance)라고 부른다
...
// 실제로 존재하는 객체를 하나 참조하여 클래스를 생성한 후 테스트
class Cafe {
		...
		for (int i = 0; i < menu.length; ++i) {
			if (menu[i] != null) {
				System.out.printf("- %s : %d원(%dkcal)\n",
						menu[i].name, menu[i].price, menu[i].caloried);				
			} else {
